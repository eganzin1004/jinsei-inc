...
#   3. 使い捨てファイルの掃除（当日分以外の calendar-*、7日超の .playwright-mcp/）
...
LOCK="/tmp/company-auto-maintain.lock"
TODAY=$(date +%Y-%m-%d)
THIS_WEEK=$(date +%G-W%V)
START_MARK="/tmp/company-auto-maintain.start"
...
trap 'rm -rf "$LOCK" "$START_MARK"' EXIT
touch "$START_MARK"
...
if [ "$HAS_CLAUDE" = 1 ] && [ ! -f "$C/secretary/notes/calendar-${TODAY}.md" ]; then
  ...
  MCPCFG=$(python3 -c "import json; d=json.load(open('$HOME/.claude.json')); print(json.dumps({'mcpServers':{'mcp-google':d['projects']['<YOUR_WORKSPACE_PATH>']['mcpServers']['mcp-google']}}))" 2>/dev/null)
  if [ -z "$MCPCFG" ]; then
...
【重要】日次ログの削除と git commit はやらないこと（呼び出し元スクリプトが検証後に行う）。" >/dev/null 2>&1
  ...
  # 検証してから削除: 該当週の weekly が今回更新され、日次ログ名が記載されていること
  removed=""; kept=""
  for d in $stale; do
    w=$(date -j -f "%Y-%m-%d" "$d" +%G-W%V 2>/dev/null)
...
# Playwright MCP のセッションフォルダ（yymmddHHMM名・使い捨てスクショ）は7日より古いものを削除
PW="$(dirname "$C")/.playwright-mcp"
if [ -d "$PW" ]; then
  pw_cutoff=$(date -v-7d +%y%m%d%H%M)
  for d in "$PW"/*/; do
    [ -d "$d" ] || continue
    n=$(basename "$d")
    case "$n" in *[!0-9]*) continue ;; esac
    [ "$n" -lt "$pw_cutoff" ] && rm -rf "$d" && cleaned=$((cleaned+1))
  done
fi
...
if [ "${changes:-0}" -gt 0 ]; then
  ...
  REF="/tmp/company-auto-maintain.ref"
